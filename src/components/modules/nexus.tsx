...
const Nexus: React.FC<{ credentials: Credentials; permissions: any }> = ({ credentials, permissions }) => {
    ...
    const handleSaveLawsuit = async (e: React.FormEvent) => {
        ...
        if (editingLawsuit?.cnj_number && !cnjRegex.test(editingLawsuit.cnj_number)) {
            alert('Número CNJ Inválido! Use o formato: 0000000-00.0000.0.00.0000');
            return;
        }
    ...
    const handleSoftDeleteLawsuit = async (id: string) => {
        if (!window.confirm('Tem certeza que deseja arquivar este processo? Ele poderá ser recuperado pelo administrador.')) return;
        try {
    ...
    return (
        <div className="flex flex-col h-full space-y-6 high-density">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Core Operacional & Gestão Processual</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center justify-between">
                    <div>
                        <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest">Prazos Fatais 24h</p>
                ...
                <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center justify-between">
                    <div>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Pendências Médias</p>
                ...
                <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center justify-between">
                    <div>
                        <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Taxa de Conclusão</p>
                ...
                {columns.map((column) => (
                    <div key={column} className="flex-shrink-0 w-80 bg-slate-100/40 dark:bg-slate-950/40 rounded-3xl p-4 border border-slate-200 dark:border-slate-900 flex flex-col gap-4">
                        ...
                        <div className="flex-1 space-y-4 overflow-y-auto no-scrollbar">
                            {loading ? (
                                <div className="py-8 text-center text-slate-400 text-xs font-bold animate-pulse">Carregando...</div>
                                    ...
                                    <div key={task.id} className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 hover:border-indigo-300 dark:hover:border-indigo-700 transition-all cursor-grab active:cursor-grabbing group">
                                        ...
                                        <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-50 dark:border-slate-800">
                                            ...
                                            <div className="flex items-center gap-1 text-slate-400">
                                                <Calendar size={10} />
                                                <span className="text-[10px] font-bold">{new Date(task.due_date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })}</span>
                                            </div>
                                        </div>
                                    </div>
                                );
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 max-h-[90vh] flex flex-col">
                        <div className="p-8 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50">
                            <div>
                                <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tighter">Cadastro de Processo</h3>
                                <p className="text-xs text-slate-500 font-medium">Vinculação relacional para automação Sentinel PRO.</p>
                            </div>
                        ...
                        <form onSubmit={handleSaveLawsuit} className="p-8 space-y-6 overflow-y-auto no-scrollbar">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="md:col-span-2">
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Número do Processo (CNJ - Estrito)</label>
                                    <input
                                        required
                                ...
                                <div className="md:col-span-2">
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Título do Caso / Identificação</label>
                                    <input
                                        ...
                                        placeholder="Ex: Ação de Cobrança - [Nome do Cliente]"
                                    />
                                </div>

                                <div>
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Polo Ativo (Autor)</label>
                                    <select
                                        required
                                ...
                                <div>
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Polo Passivo (Réu)</label>
                                    <select
                                        required
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    >
                                        {expandedLawsuitFields ? 'Ocultar Campos Avançados' : 'Ver Campos Avançados'}
                                ...
                                {expandedLawsuitFields && (
                                    <>
                                        <div>
                                            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Advogado Responsável</label>
                                            <select
                                                required
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Esfera</label>
                                                <input
                                            ...
                                            <div>
                                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Valor da Causa (R$)</label>
                                                <input
                                                    type="number"
                            ...
                            <div className="pt-8 flex gap-4">
                                <button type="button" onClick={() => setIsLawsuitModalOpen(false)} className="flex-1 px-8 py-4 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-2xl font-black uppercase tracking-widest hover:bg-slate-200 transition-all">Cancelar</button>
                                <button type="submit" className="flex-[2] px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase tracking-widest hover:bg-indigo-700 shadow-xl shadow-indigo-600/30 transition-all flex items-center justify-center gap-2">
                                    <Save size={20} /> Salvar Processo no Nexus
                                </button>
                            </div>
                        </form>
                    </div>
            ...
            {isTaskModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800">
                        <div className="p-8 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50">
                            <div>
                                <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tighter">Agendar Tarefa</h3>
                                <p className="text-xs text-slate-500 font-medium">Vinculação estrita: Equipe & Processos.</p>
                            </div>
                        ...
                        <form onSubmit={handleSaveTask} className="p-8 space-y-6">
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Vincular a Processo (Nexus)</label>
                                    <select
                                        required
                                ...
                                <div>
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Responsável (Equipe)</label>
                                    <select
                                        required
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Prazo (Due Date)</label>
                                        <input
                                            required
                                            type="datetime-local"
                            ...
                            <div className="pt-4 flex gap-4">
                                <button type="button" onClick={() => setIsTaskModalOpen(false)} className="flex-1 px-8 py-4 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-2xl font-black uppercase tracking-widest hover:bg-slate-200 transition-all text-xs">Cancelar</button>
