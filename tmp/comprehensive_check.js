...
async function check() {
    ...
    const { data: subs } = await supabase.from('user_subscriptions').select('id, user_id, plan_id, status').eq('user_id', userId);
    console.log('Subscriptions List:', JSON.stringify(subs, null, 2));
